...
function RecCard({
  ...
  return (
    <Link
      ...
      className="group relative overflow-hidden rounded-2xl border border-white/10 bg-[#080808] transition active:border-white/30"
      style={{
        boxShadow: `0 0 0 1px rgba(255,255,255,0.03), 0 24px 60px rgba(0,0,0,0.45)`,
      }}
    >
      <div
        className="pointer-events-none absolute -right-8 -top-8 h-28 w-28 rounded-full opacity-40 blur-2xl transition group-active:opacity-70"
        style={{ background: accent }}
      />

      <div className="relative aspect-[3/4] overflow-hidden">
        <div
          className="absolute inset-0 bg-cover bg-center transition duration-700 group-active:scale-105"
          style={
            thumb
    ...
    <div className="relative">
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h2 className="mt-2 text-[28px] font-black tracking-tight text-white">相似推荐</h2>
          <p className="mt-2 text-[13px] leading-6 text-white/45">
            按动作路径、舞种与好友练习圈挑出的延伸曲目。
          </p>
        </div>
        <div className="rounded-full border border-[#ccff00]/30 bg-[#ccff00]/8 px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#ccff00]">
          {recs.length} matches
        </div>
      </div>

      {recs.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-white/15 px-4 py-16 text-center text-[13px] text-white/40">
          暂时没有相似推荐
        </div>
      ) : (
