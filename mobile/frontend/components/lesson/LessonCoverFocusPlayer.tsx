...
/** 竖屏课程封面：点击全屏预览，底部叠进度/倍速/骨架；有 particle_url 时叠粒子层 */
export function LessonCoverFocusPlayer({
  ...
  const activeKey = activeKeyId ? keyActions.find((k) => k.id === activeKeyId) ?? null : null;
  ...
  const stage = (
    <div
      className={cn(
        ...
        focused
          ? "max-h-[min(92dvh,780px)] border-[#00f3ff]/45 shadow-[0_40px_90px_rgba(0,243,255,0.18)]"
          : "cursor-pointer border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.45)] active:border-[#00f3ff]/35",
        className
      )}
      onClick={() => {
        if (!focused) onFocus();
      ...
      aria-label={focused ? undefined : "点击封面，全屏预览"}
    >
      <video
        ref={videoRef}
      ...
      {focused && activeSeg?.particle_url ? (
        <SegmentParticleLayer
          src={activeSeg.particle_url}
          segStart={activeSeg.start}
          teacherTime={currentTime}
          playing={playing}
        />
      ) : null}
      ...
      {focused ? (
        ...
        >
          ...
          <div className="relative pt-6">
            {activeKey ? (
              <div
                className="pointer-events-none absolute bottom-full z-30 mb-2 w-[min(240px,88vw)] -translate-x-1/2 rounded-xl border border-[#ff5c8a]/40 bg-black/95 px-3 py-2 shadow-[0_12px_40px_rgba(255,0,85,0.25)]"
                style={{ left: `${Math.min(92, Math.max(8, (activeKey.timeSec / dur) * 100))}%` }}
              >
                <div className="text-[10px] font-bold leading-snug text-[#ff9cbb]">
                  {activeKey.hoverTitle}
                </div>
                <p className="mt-1 text-[9px] leading-relaxed text-white/60">{activeKey.hoverDetail}</p>
                <div className="mt-1 text-[8px] uppercase tracking-wider text-white/35">
                  关键 · {activeKey.label}
                  {activeKey.source === "demo" ? " · 路演示意" : " · 跟练统计"}
                </div>
              </div>
            ) : null}

            <div
              className="relative h-2.5 touch-none rounded-full bg-white/15"
              onPointerDown={(e) => {
                setDragging(true);
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      active
                        ...
                        : "border-[#ff9cbb] bg-[#ff0055]/90 shadow-[0_0_10px_rgba(255,0,85,0.55)]"
                    )}
                    style={{ left: `${left}%` }}
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      setActiveKeyId((prev) => (prev === k.id ? null : k.id));
  ...
  if (focused) {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 px-4 backdrop-blur-sm">
        <div className="w-full max-w-md">{stage}</div>
      </div>
    );
  }
