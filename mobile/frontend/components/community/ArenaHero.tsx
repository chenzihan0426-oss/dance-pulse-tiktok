...
export function ArenaHero({ challenge }: { challenge: WeeklyChallenge }) {
  ...
  return (
    <section className="arena-hero relative isolate overflow-hidden border border-white/10 animate-[arenaIn_0.7s_ease-out_both]">
      ...
      <div className="relative flex min-h-[380px] flex-col justify-end gap-6 px-5 py-6">
        <div>
          ...
          <h2
            className="mt-4 max-w-3xl text-[34px] font-black leading-[0.95] tracking-tight text-white"
            style={{ fontFamily: "'Black Han Sans', 'Noto Sans SC', sans-serif", transform: "skewX(-6deg)" }}
          >
            {challenge.lessonTitle.split(" - ")[0]}
            <span className="mt-2 block text-[16px] font-bold tracking-[0.08em] text-white/55">
              本周同舞竞技场
            </span>
          </h2>

          <div className="mt-5 flex flex-wrap items-center gap-4 text-[13px] text-white/65">
            <span className="inline-flex items-center gap-1.5">
        ...
        <div className="relative w-full border border-[#ccff00]/35 bg-black/55 p-4 backdrop-blur-md">
          <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#ccff00]/80">
            Top Score
          </div>
          <div
            className="mt-2 font-mono text-[56px] font-black leading-none tracking-tighter text-white"
            style={{ textShadow: "0 0 28px rgba(204,255,0,0.35)" }}
          >
            {challenge.topScore}
          </div>
          <div className="mt-1 text-[12px] text-white/45">本周最高分 · 可被超越</div>
          <div className="mt-3 h-px w-full bg-gradient-to-r from-[#ccff00]/70 to-transparent" />
          <div className="mt-3 text-[13px] text-white/75">{challenge.topDisplayName}</div>
        </div>

        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <Link
            href={`/lesson/${challenge.lessonId}/tracking`}
            className="inline-flex min-h-[48px] items-center justify-center gap-2 bg-[#ccff00] px-6 py-3 text-[14px] font-bold uppercase tracking-wider text-black transition active:bg-white"
          ...
          <Link
            ...
            className="inline-flex min-h-[48px] items-center justify-center gap-2 border border-white/25 bg-black/35 px-6 py-3 text-[14px] font-semibold text-white/90 backdrop-blur transition active:border-[#00f3ff]/50"
            style={{ transform: "skewX(-8deg)" }}
          >
