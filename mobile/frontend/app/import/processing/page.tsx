  ...
  return (
    <main className="relative mx-auto min-h-screen max-w-md px-5 pb-16 pt-8 text-white">
      ...
      <section className="relative">
        ...
        <h1 className="mt-4 text-[32px] font-black leading-[1.08] tracking-tight">
          <span className="bg-gradient-to-r from-[#ff0055] via-[#ffaa00] to-[#00f3ff] bg-clip-text text-transparent">
            {title}
          </span>
        </h1>
        <p className="mt-4 text-[15px] font-semibold leading-7 text-white/85">{body}</p>
      ...
      <section className="relative mt-10 overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.06] to-transparent p-5 shadow-[0_30px_80px_rgba(0,0,0,0.45)] backdrop-blur-md">
        ...
        <div className="relative">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="font-mono text-[22px] font-bold tabular-nums text-[#00f3ff]">
              {Math.round(progress)}%
            </div>
          </div>
          ...
          {(failed || !jobId) && (
            <div className="mt-6 flex flex-col gap-3">
              <button
                type="button"
                onClick={onRetry}
                className="flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#ff0055] via-[#9d4edd] to-[#00f3ff] px-4 py-3 text-[13px] font-bold text-white"
              >
              ...
              {failed ? (
                <button
                  type="button"
                  onClick={onLearn}
                  className="flex w-full items-center justify-center gap-2 rounded-2xl border border-white/15 bg-white/5 px-4 py-3 text-[13px] font-semibold text-white/80"
                >
                  返回学习页
                </button>
              ) : null}
