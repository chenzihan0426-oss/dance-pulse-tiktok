    ...
    >
      <div
        className={cn(
          ...
          compact ? "h-28 w-28 rounded-[22px]" : "w-32 sm:w-40 md:w-44"
        )}
      >
        ...
        />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(66,37,87,0.24)_0%,rgba(31,20,45,0.58)_100%)]" />
        {!compact && (
          <>
          ...
          <p className={cn("mt-1 leading-6 text-white/45", compact ? "text-[13px]" : "text-sm")}>
            {compact
              ? resumeMeta
                ...
                : `${lesson.bpm} BPM`
              : `${Math.round(lesson.duration)} 秒 · ${lesson.confirmed ? "卡片教程页" : "建议先确认切片"}`}
          </p>
        </div>
        ...
        {compact ? (
          ...
          <>
            ...
            <div className="mt-4 flex items-center justify-between text-sm font-medium text-white">
              <span>
                {resumeMeta ? "继续学习" : learnedCount >= totalCount && totalCount > 0 ? "回看课程" : "进入课程"}
              </span>
              <ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" />
            </div>
          </>
