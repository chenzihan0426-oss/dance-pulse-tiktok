...
export default function MatteOverlay({
  ...
  React.useEffect(() => {
    ...
    const animate = () => {
      if (disposed) return;
      // 动态 shimmer:轻微呼吸 + 从上到下扫光
      const t = (performance.now() - startTime) / 1000;
      // 呼吸周期 2.4s
      material.uniforms.uShimmer.value = 0.5 + 0.5 * Math.sin((t / 2.4) * Math.PI * 2);
      // 扫光周期 3.2s,y 从 -0.15 到 1.15 (多一点边界确保扫出画面)
      const sweep = ((t % 3.2) / 3.2) * 1.3 - 0.15;
      material.uniforms.uShimmerSweep.value = sweep;
