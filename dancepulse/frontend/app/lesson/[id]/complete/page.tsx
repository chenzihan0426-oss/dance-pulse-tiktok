...
export default function LessonCompletePage() {
  ...
  if (loading) {
    return (
      <main className="min-h-[100dvh] bg-bg-root px-6 py-10 text-white animate-[fadeIn_0.3s_ease-out]">
        <div className="mx-auto h-64 max-w-md animate-pulse rounded-[32px] bg-bg-raised" />
      </main>
    );
  }
  ...
  if (error || !lesson || !firstSegment) {
    return (
      <main className="min-h-[100dvh] bg-bg-root px-6 py-10 text-white animate-[fadeIn_0.3s_ease-out]">
        <div className="mx-auto max-w-md rounded-[24px] border border-state-danger/20 bg-state-danger/10 px-5 py-5 text-sm text-red-200">
          {error ?? "完课页加载失败"}
        </div>
      </main>
    );
