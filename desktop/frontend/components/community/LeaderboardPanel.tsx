...
function RankRow({ row, emphasize }: { row: LeaderboardRow; emphasize?: boolean }) {
  ...
  const inner = (
    <div
      className={`flex items-center gap-4 border px-4 py-3 transition ${
        emphasize
          ? "border-[#ccff00]/35 bg-[#ccff00]/8"
          : "border-white/8 bg-black/35 hover:border-white/18 hover:bg-black/50"
      }`}
    >
      <div className="w-8 text-center font-mono text-[20px] font-black" style={{ color: accent }}>
        {row.rank}
      </div>
      ...
      <div className="text-right">
        <div className="font-mono text-[22px] font-black text-white">{row.score || "—"}</div>
    ...
    <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      ...
      <section className="space-y-5">
        {SONG_CHALLENGE_BOARDS.map((board) => (
          <div key={board.lessonId} className="border border-white/10 bg-black/40 p-5 backdrop-blur-sm">
            <div className="flex items-start justify-between gap-3">
              ...
              <Link
                href={`/lesson/${board.lessonId}/tracking-desktop`}
                className="bg-[#ccff00] px-3 py-1.5 text-[11px] font-bold text-black transition hover:bg-white"
