...
function ArenaCard({
  ...
  return (
    <Link
      ...
      className={`arena-card group relative block overflow-hidden bg-black/50 animate-[arenaCardIn_0.55s_ease-out_both] ${
        isHero ? "md:col-span-2 md:row-span-2" : variant === "tall" ? "md:row-span-2" : ""
      }`}
      style={{
    ...
    >
      <div
        className={`relative w-full overflow-hidden bg-cover bg-center transition-transform duration-500 group-hover:scale-[1.04] ${
          isHero ? "aspect-[16/11] md:aspect-auto md:h-full md:min-h-[480px]" : variant === "tall" ? "aspect-[3/4]" : "aspect-[9/14]"
        }`}
        style={
          thumb
      ...
      <div className={`absolute left-3 top-3 ${isHero ? "left-5 top-5" : ""}`}>
        <div
          className={`font-mono font-black leading-none tracking-tighter text-white transition duration-300 group-hover:brightness-125 ${
            isHero ? "text-[64px] md:text-[84px]" : "text-[28px]"
          }`}
      ...
      <div className={`absolute inset-x-0 bottom-0 ${isHero ? "p-6" : "p-3.5"}`}>
        <div className={`line-clamp-1 font-semibold text-white ${isHero ? "text-[22px]" : "text-[14px]"}`}>
          {item.lessonTitle}
        </div>
        {meta?.caption ? (
...
export function CommunityFeedGrid({ items }: { items: CommunityFeedItem[] }) {
  ...
  return (
    <div className="arena-feed grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
